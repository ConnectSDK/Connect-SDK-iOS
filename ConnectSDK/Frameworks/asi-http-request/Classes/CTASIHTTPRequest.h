...
// The error domain that all errors generated by CTASIHTTPRequest use
extern NSString* const CTNetworkRequestErrorDomain;
...
@interface CTASIHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the CTASIHTTPRequestDelegate protocol
	id <CTASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but CTASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <CTASIHTTPRequestDelegate, CTASIProgressDelegate> queue;
	...
	// Set to true when CTASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	id <CTASIProgressDelegate> uploadProgressDelegate;
	...
	id <CTASIProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force CTASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, CTASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, CTASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - CTASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
    ...
    CTASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in CTASINetworkQueue.h
	...
	// Tells CTASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let CTASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// CTASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops CTASIHTTPRequest cleaning up a temporary post body)
	...
	// The download cache that will be used for this request (use [CTASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <CTASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See CTASICacheDelegate.h for possible values
    CTASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See CTASICacheDelegate.h for possible values
    CTASICacheStoragePolicy cacheStoragePolicy;
    ...
    CTASIDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using CTASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
    ...
    CTASIHTTPRequest *PACFileRequest;
...
// On iPhone, CTASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (atomic, assign) CTASIHTTPRequest *mainRequest;
...
@property (atomic, assign) id <CTASICacheDelegate> downloadCache;
@property (atomic, assign) CTASICachePolicy cachePolicy;
@property (atomic, assign) CTASICacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (atomic, retain) CTASIDataDecompressor *dataDecompressor;
